...
const EventDetails = () => {
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      const options = {
        key: 'rzp_test_GO2sh8z2X3t7At', // Replace with your Razorpay key
        amount,
        currency,
        name: yatraRegistrationData.fullName,
        description: '7 Yatra 2025 Registration',
        order_id,
        handler: async function (response) {
          // On payment success
          const { razorpay_order_id, razorpay_payment_id, razorpay_signature } = response;
          // Retrieve registration data and image from localStorage
          const regData = JSON.parse(localStorage.getItem('yatrikRegistrationData'));
          const photoFile = yatraRegistrationData.yatrikPhoto;
          const formData = new FormData();
          formData.append('orderId', razorpay_order_id);
          formData.append('paymentId', razorpay_payment_id);
          formData.append('signature', razorpay_signature);
          formData.append('name', regData.fullName);
          formData.append('mobileNumber', regData.phone);
          formData.append('whatsappNumber', regData.whatsappNumber);
          formData.append('emailAddress', regData.email);
          formData.append('education', regData.education);
          formData.append('religiousEducation', regData.religiousEducation);
          formData.append('weight', regData.weight);
          formData.append('height', regData.height);
          formData.append('dob', regData.dateOfBirth);
          formData.append('address', regData.address);
          formData.append('city', regData.city);
          formData.append('state', regData.state);
          formData.append('familyMemberName', regData.familyMemberName);
          formData.append('relation', regData.familyMemberRelation);
          formData.append('familyMemberWANumber', regData.familyMemberWhatsapp);
          formData.append('emergencyNumber', regData.emergencyNumber);
          formData.append('is7YatraDoneEarlier', regData.done7YatraEarlier);
          formData.append('earlier7YatraCounts', regData.howManyTimes);
          formData.append('howToReachPalitana', regData.howToReachPalitana);
          formData.append('yatrikConfirmation', regData.yatrikConfirmation);
        ...
        prefill: {
          name: yatraRegistrationData.fullName,
          email: yatraRegistrationData.email,
          contact: yatraRegistrationData.phone,
        },
        theme: { color: '#800000' },
      };
